...
* Download (and decompress):[^ftp]
    ...
    * 2013 5-year PUMS person records for Washington DC: [csv_pdc.zip](http://www2.census.gov/programs-surveys/acs/data/pums/2013/5-Year/csv_pdc.zip) (5&nbsp;MB compressed, 30&nbsp;MB decompressed)
    * 2013 5-year PUMS housing records for Washington DC: [csv_hdc.zip](http://www2.census.gov/programs-surveys/acs/data/pums/2013/5-Year/csv_hdc.zip) (2&nbsp;MB compressed, 13&nbsp;MB decompressed)
...
* Load the files:
    * Data dictionary TXT: `dsdemos.census.parse_pumsdatadict` TODO: link.
    * Person/housing and verification CSVs: `pandas.read_csv` [link](http://pandas.pydata.org/pandas-docs/stable/generated/pandas.read_csv.html)[^pd_py35]
* Verify calculations of estimates.[ Example:

---

To replicate the calculations in this row from the verification file:
...
With the verification file loaded as `dfe`, person records file loaded as `dfp`, and `import numpy as np`:  

```python
# Select the verification estimates for Washington DC then for 'Age 25-34'.
# Select the corresponding data from the person records file.
...
# Calculate the estimate ('est', also a "characteristic"),
# a weighted sum, using sample weights 'PWGTP'.
ref_est = int(dfe_dc.loc[tfmask_ref, 'pums_est_09_to_13'].values[0].replace(',', ''))
test_est = dfp.loc[tfmask_test, 'PWGTP'].sum()
assert np.isclose(ref_est, test_est, rtol=0, atol=1)
# Calculate the direct standard error ('se') using replicate weights '[PWGTP[1-80]'.
# wts = ['PWGTP1', 'PWGTP2', ..., 'PWGTP80'] are the per-person
...
* For the `dsdemos` source code, see TODO: link.
...
* TODO
    ...
    * ETL: docs, data dict, zip file, check user verification files, format data dictionary file, imputation (knn?)
        * Download the [ACS data via FTP](https://www.census.gov/programs-surveys/acs/data/data-via-ftp.html):[^no-api]  
        ...
        `http://www2.census.gov/programs-surveys/acs/data/pums/2013/5-Year/csv_hdc.zip` (unzip with `unzip`)
...
[^ftp]:
    About accessing [ACS data via FTP](https://www.census.gov/programs-surveys/acs/data/data-via-ftp.html).
[^pd_py35]:
