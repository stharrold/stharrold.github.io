...
def parse_pumsdatadict20092013(path:str) -> collections.OrderedDict:
    r"""Parse the ACS PUMS Data Dictionary for 2009-2013.
    
    Args:
        path (str): Path to downloaded PUMS_Data_Dictionary_2009-2013.txt.[^url]
    ...
    Notes:
        * Values are all strings. No data types are inferred from the
            original file.
        * Example structure of returned `ddict`:
            ddict['title'] = '2009-2013 ACS PUMS DATA DICTIONARY'
            ddict['date'] = 'August 7, 2015'
            ddict['record_types']['HOUSING RECORD']['RT']\
                ['length'] = '1'
                ['description'] = 'Record Type'
    ...
    References:
        [^url]: http://www2.census.gov/programs-surveys/acs/tech_docs/pums/data_dict/PUMS_Data_Dictionary_2009-2013.txt
    
    """
    # Check arguments.
    utils.check_arguments(
        antns=parse_pumsdatadict20092013.__annotations__,
        lcls=locals())
    if not os.path.exists(path):
        raise FileNotFoundError(
            "Path does not exist:\n{path}".format(path=path))
    ...
    with open(path, encoding='utf-8', errors='replace') as fobj:
        ...
        for line in fobj:
            ...
            # Variable note is followed by newline.
                ...
                # population.  Use with ST for unique code. PUMA00 applies to data ...
                # ...
                # """
                elif catch_var_note:
                    var_notes = line.strip() # type(var_notes) == str
                ...
                # Example: "NWAB       1 (UNEDITED - See 'Employment Status Recode' (ESR))"
                    ...
                    # type(var_notes) == list
                    (var_name, var_len, *var_notes) = line.strip().split(maxsplit=2)
                    ddict['record_types'][record_type][var_name] = collections.OrderedDict()
                    ddict['record_types'][record_type][var_name]['length'] = var_len
                    if len(var_notes) > 0:
                        if 'notes' not in ddict['record_types'][record_type][var_name]:
